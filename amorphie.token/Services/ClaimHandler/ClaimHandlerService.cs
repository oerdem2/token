    ...
    public class ClaimHandlerService : ServiceBase, IClaimHandlerService
    {
        ...
        public async Task<Claim?> GetClaimDetail(string[] claimPath)
        {
            ...
            if (claimPath.First().Equals("profile"))
            {
                ...
                if (propValue != null)
                    return new Claim(claimName, propValue!.ToString()!);
                else
                    return null;
        ...
        public async Task<KeyValuePair<string,object>?> GetKeyVal(string[] claimPath)
        {
            var claimName = string.Empty;
            var claimType = string.Empty;
            if (claimPath.First().Contains("|"))
            ...
            if (claimPath.First().Equals("tag"))
            {
                try
                {
                    ...
                    return new KeyValuePair<string,object>(claimName, tagData[fieldName].ToString());
            ...
            if (claimPath.First().Equals("user"))
            {
                ...
                return new KeyValuePair<string,object>(claimName, property!.GetValue(_user!)!);
            }
            ...
            if (claimPath.First().Equals("openbanking"))
            {
                ...
                return new KeyValuePair<string,object>(claimName, property!.GetValue(_consent)!);
            }
            ...
            if (claimPath.First().Equals("profile"))
            {
                ...
                if (propValue != null)
                    return new KeyValuePair<string,object>(claimName, propValue!);
                else
                    return null;
            ...
            if (claimPath.First().Equals("const"))
            {
                 return new KeyValuePair<string,object>(claimName, claimPath[1]);
            }
        ...
        public async Task<List<KeyValuePair<string, object>?>> PopulatePrivateClaims(List<string> clientClaims, LoginResponse? user, SimpleProfileResponse? profile = null, ConsentResponse? consent = null, core.Models.Collection.User? collectionUser = null)
        {
            ...
            foreach (var identityClaim in clientClaims)
            {
                ...
                var claimValue = await GetKeyVal(claimInfo);
                ...
                {
                    claimInfo = alternativeClaim.Split(".");
                    claimValue = await GetKeyVal(claimInfo);
                    if (claimValue != null)
                    {
                        claims.Add(claimValue);
                    }
